        ...
        <div class="grid grid-cols-2 px-[255px] py-[80px] bg-[#14141f] gap-y-[230px] relative isolate">
            <div class="flex flex-col gap-y-8 max-w-[633px] relative">
                ...
                <div class="flex gap-x-4">
                    ...
                    <a href="{{ action([\App\Http\Controllers\ItemController::class, 'create']) }}" class="flex gap-x-2 px-[37px] py-[17px] border-[1px] bg-white text-[#343444] font-bold rounded-[30px]">
...
{{--                Container principal de las cards--}}
                <article class="flex flex-col gap-y-[15px] p-5 bg-[#343444] w-[450px] h-[395px] rounded-[10px]">
                    <div class="grid grid-cols-6 gap-[10px] w-[410px] h-[272px]">
                        @foreach($collection->items as $item)
                            ...
                            @elseif($loop->iteration <=5)
                                <div class="bg-[#7A798A] col-span-2 h-[131px] rounded-[10px]"><img src="{{$item->getFirstMediaUrl('image_items')}}" alt="" class="w-full object-center object-cover col-span-2 h-[131px] rounded-[10px]"></div>
                            @else
                                @break
                            @endif
                        @endforeach
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div onclick="likeCollection({{$collection->id}});" class="flex gap-x-[5px] items-center text-white px-[11px] py-[2px] rounded-[10px] bg-[#14141F] mt-[-20px] hover:cursor-pointer">
                            ...
                            <span id="counter_collection_likes_id_{{$collection->id}}">{{$collection->likes()->count()}}</span>
                        </div>
                    </div>
                </article>
            @endforeach
