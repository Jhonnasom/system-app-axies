    ...
    <main class="bg-[#14141f] py-[80px] px-[255px] flex flex-col">
        <section class="flex gap-x-10 bg-[#414046] p-10 rounded-[12px] relative isolate overflow-hidden">
            <div class="w-[274px] h-[274px] rounded-[20px] bg-[#7A798A]">
                <img class="w-full object-cover object-center rounded-[20px]" src="{{$user->getFirstMediaUrl('profile')}}" alt="Author image">
            </div>
            ...
            <nav class="bg-[#343444] w-full absolute bottom-0 left-0 px-[100px] py-[24px] z-[-1] flex justify-end">
                <ul class="flex items-center w-[896px] justify-around">
                    <li><a href="/home/author/{{$user->id}}" class="text-white font-bold uppercase text-[20px]">All</a></li>
                    @foreach($categories as $category)
                        <li><a href="/home/author/{{$user->id}}?category={{$category->id}}" class="text-white font-bold uppercase text-[20px]">{{$category->name}}</a></li>
                    @endforeach
                </ul>
            </nav>
        </section>
